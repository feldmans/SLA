...
all_SLA$censor <- ifelse (!is.na(all_SLA$date_dc),1, 0)
# sex : 1 = 'Masculin' 2 = 'Feminin' => 0 = 'Masculin' 1 = 'Feminin'
...
```{r, echo=FALSE, message=FALSE}
#suivi
sym.suiv2 <- survfit(Surv((time.vni*12/365.25),1-censor)~1,data=s)
...
```{r}
s<-sla[!is.na(sla$time.vni),]
vni.surv <- survfit(Surv(time.vni,censor)~1, data=s, conf.int=.95)
...
## Familial
```{r}
#check_RP (var="familial", data=sla, .time="time.vni", .censor="censor", type="quali")
draw_surv_bin(var="familial", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", surv_only = TRUE) 
Test_score_HR_IC(var="familial", data=sla, .time="time.vni", .censor="censor", type="quali", dep_temps = FALSE)
```
...
## Sex
```{r, eval=FALSE}
check_RP (var="sex", data=sla, .time="time.vni", .censor="censor", type="quali")
#tmp <- lapply(c("log","sqrt","*t","/t","2sqrt"),function(x){
tmp <- lapply("*t^0.7",function(x){
add_vart_and_check (var="sex", data=sla, .time="time.vni", .censor="censor", type="quali", recode=FALSE, .transf=x)  
})
...
```{r}
draw_surv_bin(var="sex", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", surv_only = TRUE) 
```
0: male 1: female

```{r}
Test_score_HR_IC(var="sex", data=sla, .time="time.vni", .censor="censor", type="quali", recode=FALSE, dep_temps = TRUE, .transf="*t^0.7", vec_time=12)
```

## Riluzole
```{r, eval=FALSE}
check_RP (var="rilu", data=sla, .time="time.vni", .censor="censor", type="quali")
```

Proportionnal HR hypothesis verified
```{r}
draw_surv_bin(var="rilu", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", surv_only = TRUE) 
```


```{r}
Test_score_HR_IC(var="rilu", data=sla, .time="time.vni", .censor="censor", type="quali", dep_temps = FALSE)
```

## Rest dyspnoea

```{r, eval=FALSE}
check_RP (var="dysp", data=sla, .time="time.vni", .censor="censor", type="quali")
tmp <- lapply(c("log"),function(x){
add_vart_and_check (var="dysp", data=sla, .time="time.vni", .censor="censor", type="quali", recode=FALSE, .transf=x)  
})
cat(do.call(rbind,tmp))
```
...
```{r}
draw_surv_bin(var="dysp", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", surv_only = TRUE) 
```
```{r}
Test_score_HR_IC(var="dysp", data=sla, .time="time.vni", .censor="censor", type="quali", recode=FALSE, dep_temps = TRUE, .transf="log", vec_time=24)
```
...
```{r, eval=FALSE}
check_RP (var="orthop", data=sla, .time="time.vni", .censor="censor", type="quali")
Test_score_HR_IC(var="orthop", data=sla, .time="time.vni", .censor="censor", type="quali", recode=FALSE, dep_temps = FALSE)
```
```{r}
draw_surv_bin(var="orthop", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", surv_only = TRUE)
```
...
add_vart_and_check (var="llimb", data=s, .time="time.vni", .censor="censor", type="quali", recode=FALSE, .transf=x)
})
cat(do.call(rbind,tmp))
...
Test_score_HR_IC(var="LIEUDEB_recode", data=sla, .time="time.vni", .censor="censor", type="quali2", recode=FALSE, dep_temps = FALSE)
```

## agevni

```{r}
# check_loglin (var="agevni", data=sla, .time="time.vni", .censor="censor")
# check_RP(var="agevni", data=sla, .time="time.vni", .censor="censor", type="quanti", recode = FALSE)
# tmp <- lapply(c("log","sqrt","*t","/t","*t^0.7"),function(x){
# add_vart_and_check (var="agevni", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, .transf=x)
# })
# cat(do.call(rbind, tmp))
Test_score_HR_IC(var="agevni", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, dep_temps = FALSE)
...
```{r}
#check_loglin (var="CVF_ASSIS_perc_pred", data=sla, .time="time.vni", .censor="censor")
# check_RP(var="CVF_ASSIS_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode = FALSE)
# tmp <- lapply(c("log","sqrt","*t","/t","*t^3"),function(x){
# add_vart_and_check (var="CVF_ASSIS_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, .transf=x)
# })
# cat(do.call(rbind, tmp))
Test_score_HR_IC(var="CVF_ASSIS_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, dep_temps = TRUE, vec_time=12, .transf="*t^3")
```
Loglinearity hypothesis verified.
...
```{r}
#check_loglin (var="CVF_COUCHE_perc_pred", data=sla, .time="time.vni", .censor="censor")
# check_RP(var="CVF_COUCHE_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode = FALSE)
# tmp <- lapply(c("log","sqrt","*t","/t","*t^2"),function(x){
# # tmp <- lapply(c("*t^0.7"),function(x){
#  add_vart_and_check (var="CVF_COUCHE_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, .transf=x)
#  })
# cat(do.call(rbind, tmp))
Test_score_HR_IC(var="CVF_COUCHE_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, dep_temps = FALSE)
```
Loglinearity hypothesis verified.
...
# check_RP(var="SNIP_cmH2O", data=sla, .time="time.vni", .censor="censor", type="quanti", recode = FALSE)
# tmp <- lapply(c("log","sqrt","*t","/t","*t^2"),function(x){
# # tmp <- lapply(c("*t^0.7"),function(x){
#  add_vart_and_check (var="SNIP_cmH2O", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, .transf=x)
#  })
# cat(do.call(rbind, tmp))
Test_score_HR_IC(var="SNIP_cmH2O", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, dep_temps = TRUE, vec_time=12, .transf="*t")
```
Loglinearity hypothesis verified.
...
# check_RP(var="SNIP_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode = FALSE)
# tmp <- lapply(c("log","sqrt","*t","/t","*t^2"),function(x){
# # tmp <- lapply(c("*t^0.7"),function(x){
#  add_vart_and_check (var="SNIP_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, .transf=x)
#  })
# cat(do.call(rbind, tmp))
Test_score_HR_IC(var="SNIP_perc_pred", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=FALSE, dep_temps = TRUE, vec_time=12, .transf="*t^2")
```
Loglinearity hypothesis verified.
...
# check_RP(var="PIMAX_cmH2O", data=sla, .time="time.vni", .censor="censor", type="quanti", recode = .recode)
# tmp <- lapply(c("log","sqrt","*t","/t","*t^2"),function(x){
# # tmp <- lapply(c("*t^0.7"),function(x){
#  add_vart_and_check (var="PIMAX_cmH2O", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=.recode, .transf=x)
#  })
# cat(do.call(rbind, tmp))

Test_score_HR_IC(var=.var, data=sla, .time="time.vni", .censor="censor", type="quanti", recode=.recode, dep_temps = .dep_temps, vec_time=12, .transf=.transf)
```
Loglinearity not verified. Variable transformed in binary variable (median).
Proportionnality not verified, variable corrected with *t (not perfect)

```{r}
draw_surv_bin(var=.var, data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quanti", surv_only = TRUE) 
```
...
# check_loglin (var=.var, data=sla, .time="time.vni", .censor="censor")
# check_RP(var=.var, data=sla, .time="time.vni", .censor="censor", type="quanti", recode = .recode)
# tmp <- lapply(c("log","sqrt","*t","/t","*t^3"),function(x){
#   # tmp <- lapply(c("*t^0.7"),function(x){
#   add_vart_and_check (var=.var, data=sla, .time="time.vni", .censor="censor", type="quanti", recode=.recode, .transf=x)
# })
# cat(do.call(rbind, tmp))
...
```{r}
draw_surv_bin(var=.var, data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quanti", surv_only = TRUE, pvalue = TRUE, dep_temps = TRUE, .transf="*t") 
```
...
```{r}
draw_surv_bin(var="familial", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", pvalue = TRUE) 
```
```{r}
draw_surv_bin(var="sex", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type="quali", pvalue=TRUE, dep_temps = TRUE, .transf="*t^0.7")
```
```{r}
draw_surv_bin(var="rilu", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", pvalue = TRUE) 
```
```{r}
draw_surv_bin(var="dysp", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type="quali", pvalue=TRUE, dep_temps = TRUE, .transf="log")

```
```{r}
draw_surv_bin(var="orthop", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quali", pvalue = TRUE)
...
```{r, fig.width=8, fig.height=8}
draw_surv_bin(var="PIMAX_cmH2O", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quanti", surv_only = FALSE, pvalue = TRUE, dep_temps = TRUE, .transf="*t") 
```
```{r, fig.width=8, fig.height=8}
draw_surv_bin(var="bulb_score", data=sla, .time="time.vni", .censor="censor", vec_time_IC= 1, type = "quanti", surv_only = FALSE, pvalue = TRUE)
```