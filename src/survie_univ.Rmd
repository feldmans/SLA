...
Je met option type==quanti +/- recode==TRUE

#### courbe de survie car variable recodée en binaire.
```{r, echo=FALSE}
draw_surv_bin (var="agevni", data=sla, .time="time.vni", .censor="censor", vec_time_IC= c(1, 3), type = "quanti")
```
...
```{r}
check_RP(var="agevni", data=sla, .time="time.vni", .censor="censor", type="quanti", recode = TRUE)
```
...
add_vart_and_check (var="agevni", data=sla, .time="time.vni", .censor="censor", type="quanti", recode=TRUE, .transf=x)  
})
...
at significatif avec log, je reteste les conditions(Harrel et plot)
...
#### IC : utiliser la matrice de variance covariance pour calculer la variance de l'HR si variable dependante du temps introduite
```{r, echo=FALSE}
S <- vcov(coxt)
b <- coef(coxt)
...
```{r, echo=FALSE, eval=FALSE}
#### IC : utiliser la matrice de variance covariance pour calculer la variance de l'HR si var dep du temps
