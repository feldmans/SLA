...
quanti <- c(quanti, "MRC") 
quali <- c("TOUX_EFFICACE", "LIEUDEB_recode", "LIEUDEB_rec2")
...
ggsave(file="data/analyses/courbes/20170824binaire_bl.pdf", ml)
 # 
# #----
# #survie à 1 et 3 ans
...
# df_surv <- df_surv[ , c("variable", "param", "time", "survIC")]
...
.l <- lapply(binaire, function(x)check_RP(var=x, data=d, .time="time.vni", .evt="evt", recode = FALSE, resid = FALSE))
dev.off()
...
.l <- lapply(binNRP, function(i){
  ...
  tmp <- lapply(c("log", "sqrt", "*t", "/t", "*t^2", "*t^0.3", "*t^3"), function(x)add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var=i, .transf=x))
...
dev.off()
saveRDS(df_bint, "data/df_bint.rds")
...
# check_RP(var=tmp[19], data=d, .time="time.vni", .evt="evt", recode = FALSE, .resid = FALSE, ylim = c(-2,2))
# check_RP(var="SOM_REPAR", data=d, .time="time.vni", .evt="evt", recode = FALSE, .resid = FALSE, ylim = c(-2,2))
pdf(paste0("data/analyses/RP_bin_decoup.pdf"))
transf1 <- c(3, 18)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="SEX", vec_cut = transf1)
transf2 <- c(18)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="SAS_PREEXIST_PP", vec_cut = transf2)
transf3 <- c(12)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="APPAREILLE_PP", vec_cut = transf3)
transf4 <- c(3, 18)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="DYSP_REPOS", vec_cut = transf4)
transf5 <- c(6, 30)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="DYSP_PAROLE", vec_cut = transf5)
transf6 <- c(6)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="DYSP_DECUBI", vec_cut = transf6)
transf7 <- c(4)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="DYSP_PAROX", vec_cut = transf7, ylim = c(-2,2))
transf8 <- c(4, 18, 36)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="SOM_REPAR", vec_cut = transf8, ylim = c(-2,2))
transf9 <- c(3, 18, 32)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="REVEIL_MULTI", vec_cut = transf9, ylim = c(-2,2))
transf10 <- c(4, 14, 45)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="REVEIL_ETOUF", vec_cut = transf10, ylim = c(-2,2))
transf11 <- c(6, 18, 32)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="CAUCHEMAR", vec_cut = transf11, ylim = c(-2,2))
transf12 <- c(6)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="SOM_APNEE", vec_cut = transf12, ylim = c(-2,2))
transf13 <- c(4, 16, 36)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="R_MUSCL_ACCES", vec_cut = transf13, ylim = c(-2,2))
transf14 <- c(16, 36, 41)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="OXY_THERAP", vec_cut = transf14, ylim = c(-2,2))
dev.off()
...
.l <- list(transf1, transf2, transf3, transf4, transf5, transf6, transf7, transf8, transf9, transf10, transf11, transf12, transf13, transf14)
my_transf <- sapply(seq(.l), function(i)paste(.l[[i]], collapse = "-"))
binNRPcut <- data.frame(variable = tmp, transf = my_transf, stringsAsFactors = FALSE)
...
allbin <- readRDS("data/analyses/all_var_bin_bl.rds")
...
#aucun ne convient : courbes moches 06/08/2017 
...
tmp <- QN_RP 
pdf(paste0("data/analyses/RP_quanti_bl_decoup.pdf"))
#transf1 <- c(12, 24, 60)
transf1 <- c(18, 36)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="agevni", vec_cut = transf1, ylim = c(-0.2, 0.2))
#check_RP(data=d, .time="time.vni", .evt="evt", recode = TRUE, var="SNIP_THEO",  ylim = c(-2, 2))
transf2 <- c(3, 36)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="F_RESPIR", vec_cut = transf2)
transf3 <- c(2, 18, 28)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="MRC", vec_cut = transf3, ylim = c(-1, 1))
transf4 <- c(6, 18, 30, 36)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = FALSE, var="PIMAX_THEO", vec_cut = transf4, ylim = c(-0.02, 0.02))
transf5 <- c(18)
add_vart_and_check(data=d, .time="time.vni", .evt="evt", recode = TRUE, var="SNIP_THEO", vec_cut = transf5, ylim = c(-2, 2))
dev.off()

.l <- list(transf1,transf2, transf3, transf4, transf5)
my_transf <- sapply(seq(.l), function(i)paste(.l[[i]], collapse = "-"))
QN_RPcut <- data.frame(variable = tmp$variable, recode = tmp$recode, transf = my_transf, stringsAsFactors = FALSE)
...
df_surv_quant_bl <- df_surv_quant_bl[ , c("variable", "param", "time", "survIC")]
...
quali<-c("TOUX_EFFICACE", "LIEUDEB_rec2")
#courbes de survie
.l <- lapply(quali[2], function(x) draw_surv_qualisup2(var=x, data = d, .time = "time.vni", .evt = "evt", vec_time_IC = 1 , surv_only=FALSE, pvalue=TRUE))
ml <- marrangeGrob(.l,ncol=1,nrow=1,top = NULL)
ggsave(file="data/analyses/courbes/20170825courbe_survie_qualisup2_bl.pdf", ml)

#survie
df_surv_q_bl <- lapply(quali, function(x) draw_surv_qualisup2(var=x, data = d, .time = "time.vni", .evt = "evt", vec_time_IC = 1 , surv_only=TRUE))
df_surv_q_bl <- do.call(rbind, df_surv_q_bl)
...
df_surv_q_bl <- df_surv_q_bl[ , c("variable", "param", "time", "survIC")]
...
s$a <- s[ ,var]
s <- s[!is.na(s$a),]
# cox<-coxph(Surv(tps, evt) ~ a, data=s)
# test <- summary(cox)
...
# df <- data.frame(variable = var, recode = FALSE, RP = TRUE, transf = NA, tps_clinique = NA, HRIC = HRIC, test = "score",
#                  statistic = stat, pvalue = pval, 
#                  param = gsub(var,"",names(coefbeta)), beta = coefbeta)
# df1 <- rbind(df1,df)

######################################
#modif lieudeb_rec2
#modif Non RP : lieudeb recode : a faire
s$cerv_ulimb <- ifelse(s$LIEUDEB_rec2=="cerv_ulimb",1,0)
s$llimb <- ifelse(s$LIEUDEB_rec2=="llimb", 1, 0)
s$resp <- ifelse(s$LIEUDEB_rec2=="resp", 1, 0)
...
# 
# slat$cerv_ulimb_t <- slat$cerv_ulimb * log(slat$stop) #non graphe mauvais
# coxt <- coxph(Surv(start, stop, evt) ~ cerv_ulimb +cerv_ulimb_t+ cluster(PATIENT), data=slat)
# slat$llimb_t <- slat$llimb * (slat$stop^3) #non graphe mauvais
# coxt <- coxph(Surv(start, stop, evt) ~ llimb +llimb_t+ cluster(PATIENT), data=slat)
# slat$resp_t <- slat$resp * log(slat$stop) #non graphe mauvais
# coxt <- coxph(Surv(start, stop, evt) ~ resp +resp_t+ cluster(PATIENT), data=slat)
# coxt <- coxph(Surv(start, stop, evt) ~ llimb + resp + cerv_ulimbimb + llimb_t + resp_t + cerv_ulimb_t +cluster(PATIENT), data=slat)
# coxt <- coxph(Surv(start, stop, evt) ~ llimb + resp + cerv_ulimb + resp_t + cluster(PATIENT), data=slat)
# test <- summary(coxt)
# test$coefficients
# b<-coef(coxt)
# zit<-cox.zph(coxt, "rank")
# zi<-cox.zph(coxt, "identity")
# z<-zi
# par(mfrow=c(2,3))
# for (iz in 1:5) {
#   #N  plot(z[iz], col="lightblue", ylim=c(-4,4))
#   plot(z[iz], col="lightblue", ylim = c(-5,5))
#   abline(h=b[iz], col="red")
# }
...
#for (j in myVar){
yt <- slat
for (j in c("cerv_ulimb", "resp", "llimb")){
  print(j)
...
vat<-apply(expand.grid(list(c("cerv_ulimb", "resp", "llimb"), "t", 1:(length(b)+1))),1, paste, collapse="")
...
f<-paste("Surv(start, stop, evt) ~ ", paste(wat, collapse="+")," + cluster(PATIENT)", sep="")  #on ne met pas x car les at couvre deja  toutes les perdiodes
#model
...
HRIC <- lapply(levels(s$LIEUDEB_rec2)[-1], function(lev){
  coefbeta2 <- coefbeta[grep(lev, names(coefbeta))]
  vcov2 <- diag(vcov(coxt))[grep(lev, names(coefbeta))]
  ...
  coefbetacol <- paste(coefbeta2, collapse = ";")
  name_param <- paste(names(coefbeta2), collapse = ";")
  data.frame(HRIC, coefbetacol, name_param, stringsAsFactors = F)
})
dfHR <- do.call(rbind, HRIC)

df_HR_quali_bl <-data.frame(variable = "LIEUDEB_rec2", recode = FALSE, RP = FALSE,
                            transf = transf, tps_clinique = .tps_clinique, HRIC = dfHR$HRIC, test = "robust",
                            statistic = stat, pvalue = pval, param = dfHR$name_param, beta = dfHR$coefbetacol)#HRIC : sans transformation uniquement pour l'instant
...
s$a <- s[ ,var]
s <- s[!is.na(s$a),]
s$TOUX_EFFICACE <- as.character(s$TOUX_EFFICACE)

# #interpretation si pas de modif a faire
# cox<-coxph(Surv(tps, evt) ~ a, data=s)
# test <- summary(cox)
# HRIC <- round(test$conf.int,2)
# coefbeta <- round(test$coefficients[,1],4)
...
#                  statistic = stat, pvalue = pval, 
#                  param = gsub(var,"",names(coefbeta)), beta = coefbeta)
# #Je supprime la ligne a2 qui correspond a la classe NA qu'on ne peut pas interpreter
# df <- df[1,]
# df1 <- rbind(df1,df)
# 
# saveRDS(df1, "data/analyses/HRIC_quali_bl.rds")

################33
#modif toux efficace

s$TOUX_EFFICACE1 <- ifelse(s$TOUX_EFFICACE==1, 1, 0)
s$TOUX_EFFICACE2 <- ifelse(s$TOUX_EFFICACE==2, 1, 0)
ti <- sort(unique(c(0,s$tps[s$evt==1])))
...
slat <- survSplit(slat, end="stop", event="evt", start="start",cut=ti) #Yann
#slat <- survSplit(Surv(stop,evt)~.,slat,start="start",cut=ti)           #Sarah
# slat$TOUXt1 <- slat$TOUX_EFFICACE1 * log(slat$stop) #non graphe mauvais
# slat$TOUXt1 <- slat$TOUX_EFFICACE1 /(slat$stop) #non graphe mauvais
# slat$TOUXt1 <- slat$TOUX_EFFICACE1 *(slat$stop) #non graphe mauvais
# slat$TOUXt1 <- slat$TOUX_EFFICACE1 *(slat$stop)^2 #non graphe mauvais
# slat$TOUXt1 <- slat$TOUX_EFFICACE1 *(slat$stop)^0.3 #non graphe mauvais
# slat$TOUXt1 <- slat$TOUX_EFFICACE1 * sqrt(slat$stop) #non graphe mauvais
# slat$TOUXt1 <- slat$TOUX_EFFICACE1 * (slat$stop)^3 #non graphe mauvais
# slat$TOUXt2 <- slat$TOUX_EFFICACE2 * log(slat$stop) #non graphe mauvais
# coxt <- coxph(Surv(start, stop, evt) ~  TOUXt1 + TOUX_EFFICACE1 + TOUXt2 + TOUX_EFFICACE2 + cluster(PATIENT), data=slat)#c'est pire
# coxt <- coxph(Surv(start, stop, evt) ~  TOUXt1 + TOUX_EFFICACE1 + TOUX_EFFICACE2 + cluster(PATIENT), data=slat)#c'est pire
# test <- summary(coxt)
# test$coefficients
# b<-coef(coxt)
# zit<-cox.zph(coxt, "rank")
...
# for (iz in 1:5) {
#   #N  plot(z[iz], col="lightblue", ylim=c(-4,4))
#   plot(z[iz], col="lightblue", ylim = c(-1,1))
#   abline(h=b[iz], col="red")
# }
...
yt <- slat
#for (j in paste0("TOUX_EFFICACE", 1:2)){
for (j in "TOUX_EFFICACE1"){
  print(j)
  for (i in rank_interv){
    print(i)
    if (i == 1) tmp <-  ifelse(yt$stop<=b[1], yt[ ,j], 0)
...
vat<-apply(expand.grid(list(paste0("TOUX_EFFICACE", 1:2), "t", 1:(length(b)+1))),1, paste, collapse="")
vat <- vat [vat %in% names(yt)]
x<-yt[, vat]
sx<-colSums(x) #interval de temps sans evt
wat<-vat[sx>0] #on supprime interval de temps quand pas d'evenement
#f<-paste("Surv(start, stop, evt) ~ ", paste(wat, collapse="+")," + cluster(PATIENT)", sep="")  #on ne met pas x car les at couvre deja  toutes les perdiodes
f<-paste("Surv(start, stop, evt) ~ ", paste(wat, collapse="+")," + TOUX_EFFICACE2+ cluster(PATIENT)", sep="")  #on ne met pas x car les at couvre deja  toutes les perdiodes
...
for (i in 1:(nrow(zt$table)-1)){
  iz<-i
  plot(zt[iz], resid = FALSE, main = paste0("plot shoenfeld for TOUX_EFFICACE cut ", name_cut), ylim = c(-2,2))
  abline(h=0, col="red")
  abline(h=coef(coxt)[iz], col="blue", lty=2)
  abline(v=b)
}
...
HRIC <- paste0(HRIC[1], " [", HRIC[2], " - ", HRIC[3],"]")
dfHR <- rbind(dfHR, data.frame(transf = transf, param = paste(name_param[grep("EFFICACE1", name_param)], collapse = ";"),
                               beta = paste(coefbeta[grep("EFFICACE1", name_param)], collapse = ";"),
                               HRIC = HRIC, stringsAsFactors = FALSE))
#Je n'interprete pas TOUX_EFFICACE 2 qui est une classe NA
# HRIC <- round(data.frame(IC=test$conf.int["TOUX_EFFICACE2",1], l=test$conf.int["TOUX_EFFICACE2",3], u=test$conf.int["TOUX_EFFICACE2",4]),3)
# HRIC <- paste0(HRIC[1], " [", HRIC[2], " - ", HRIC[3],"]")
# dfHR <- rbind(dfHR, data.frame(transf = NA, param = "TOUX_EFFICACE2",
#                                beta = coefbeta["TOUX_EFFICACE2"],
#                                HRIC = HRIC, stringsAsFactors = FALSE))
rownames(dfHR) <- NULL
df <- data.frame(variable = var, recode = FALSE, RP = FALSE, transf = dfHR$transf, tps_clinique = .tps_clinique, HRIC = dfHR$HRIC, test = "robust",
                 statistic = stat, pvalue = pval, param = dfHR$param, beta = dfHR$beta, stringsAsFactors = FALSE)

df_HR_quali_bl <- rbind(df_HR_quali_bl, df)
# df1 <- rbind(df1,df)
# 
...
# }
# 
# vat<-apply(expand.grid(list(c("ulimb", "resp", "cerv"), "t", 1:(length(b)+1))),1, paste, collapse="")
# vat <- vat [vat %in% names(yt)]
# x<-yt[, vat]
# sx<-colSums(x) #interval de temps sans evt
# wat<-vat[sx>0] #on supprime interval de temps quand pas d'evenement
# #f<-paste("Surv(start, stop, etat) ~ ", paste(wat, collapse="+")," + cluster(PATIENT)", sep="")  #on ne met pas x car les at couvre deja  toutes les perdiodes
# f<-paste("Surv(start, stop, evt) ~ ", paste(wat, collapse="+")," + llimb + cluster(PATIENT)", sep="")  #on ne met pas x car les at couvre deja  toutes les perdiodes
# 
# #model
# coxt <- coxph(as.formula(f), data=yt)
# 
...
# ###########
# # sarah
# slat$ulimb_t <- slat$ulimb * (slat$stop^2) #non graphe mauvais
# slat$ulimb_t <- slat$ulimb * (slat$stop) #non graphe mauvais
...
#  slat$resp_t <- slat$resp * log(slat$stop) #interactiton signif
#  # slat$cerv_t <- slat$cerv / (slat$stop) #beta ok et graphe ok qd seul
#  # slat$cerv_t <- slat$cerv * log(slat$stop) #beta ok et graphe ok qd seul
...
# #  slat$cerv_t <- slat$cerv * (slat$stop^0.3) #non graphe mauvais
# #  slat$cerv_t <- slat$cerv * (slat$stop^2) #non graphe mauvais
# # # slat$llimb_t <- slat$llimb * (slat$stop^3) #non graphe mauvais
...
# # coxt <- coxph(Surv(start, stop, evt) ~ cerv + llimb + resp + ulimb + ulimb_t +cluster(PATIENT), data=slat)
# # coxt <- coxph(Surv(start, stop, evt) ~ cerv + llimb + resp + ulimb + resp_t +cluster(PATIENT), data=slat)#corrige mais pourtant resp etait ok
# # coxt <- coxph(Surv(start, stop, evt) ~ cerv + llimb + resp + ulimb + cerv_t+resp_t +cluster(PATIENT), data=slat)#corrige mais pourtant resp etait ok
# # coxt <- coxph(Surv(start, stop, evt) ~ cerv + cerv_t + llimb + resp + ulimb + cluster(PATIENT), data=slat)#c'est pire
# # test <- summary(coxt)
# # test$coefficients
# # b<-coef(coxt)
# # zit<-cox.zph(coxt, "rank")
...
# # for (iz in 1:5) {
# #   #N  plot(z[iz], col="lightblue", ylim=c(-4,4))
# #   plot(z[iz], col="lightblue", ylim = c(-2,2))
# #   abline(h=b[iz], col="red")
# # }
# 
...
#liste des variables potentiellement intéressantes
ncu %>% filter(!grepl("MEMBRE", var), !grepl("TTT_", var), !grepl("R_CUT", var), 
               !grepl("S_HOFF*", var), !grepl("TEST_MUSCUL", var), !grepl("LIMIT_ART", var), 
               !grepl("EVAL_", var), !grepl("TREPIDATION", var), !grepl("CHUTE", var), 
               !grepl("TRONC", var),!grepl("MODIF_PARAM_SV", var) )
...
rNRP_t$var <- as.character(rNRP_t$variable)
rNRP_t <- rNRP_t[!duplicated(rNRP_t$variable),]
rNok <- full_join(rNok, rNRP_t[ ,c("var", "transf")])
...
# b <- c(6, 18, 36, 48) # moins bien que c(3, 6, 18, 36, 60)
# b <- c(3, 18, 48) # moins bien que c(3, 6, 18, 36, 60)
...
pdf(paste0("data/analyses/RP_var_rep_cut.pdf"))

var <- "CEPHAL_PP"
assign(paste0("transf",var), c(5, 18, 30))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "ENCOMBR_PP"
assign(paste0("transf",var), c(3, 6, 20))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "MORPHO_PP_CHOICE_1"
assign(paste0("transf",var), c(3, 22))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "MORPHO_PP_CHOICE_2"
assign(paste0("transf",var), c(4, 18))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "NYCTURIE_PP"
assign(paste0("transf",var), c(4 ,18, 36))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "QUALIT_SOM_PP"
assign(paste0("transf",var), c(18, 36))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "REVEIL_VENT_SV"
assign(paste0("transf",var), c(4, 22, 36))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "SOMNOLENCE_PP"
assign(paste0("transf",var), c(6, 30))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "ALS"
assign(paste0("transf",var), c(4, 22))
cut_rep(var=var, get(paste0("transf",var)), recode = TRUE)

var <- "DUREE_SOMM_VENT_SV"
assign(paste0("transf",var), c(6, 18, 36))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "E_BULBAIRE"
assign(paste0("transf",var), c(3, 24, 54))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)
...
var <- "HCO3_PP_CL1"
assign(paste0("transf",var), c(3, 22, 36))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)
...
var <- "PACO2_PP_CL1"
assign(paste0("transf",var), c(6, 22, 36))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "PAO2_PP_CL1"
assign(paste0("transf",var), c(6, 12, 22))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "PH_PP_CL1"
assign(paste0("transf",var), c(14, 24))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)

var <- "SAO2_PP"
assign(paste0("transf",var), c(16, 24))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)
...
var <- "UTIL_VENTIL_DIURN_SV"
assign(paste0("transf",var), c(6, 24, 40))
cut_rep(var=var, get(paste0("transf",var)), recode = TRUE)

var <- "UTIL_VENTIL_NOCT_SV"
assign(paste0("transf",var), c(8, 20, 36))
cut_rep(var=var, get(paste0("transf",var)), recode = FALSE)
dev.off()

#assigner la transformation a la variable correspondante
rNRP_cut$transf <- sapply(rNRP_cut$var, function(var) paste(get(paste0("transf", var)), collapse = "-"))

# .l <- list(transf1,transf2,transf3)
# rNRP_cut$transf <- sapply(1:3, function(i)paste(.l[[i]], collapse = "-"))
...
# # yt$x3t<-(yt$stop)^3*yt$x3
# # yt$x3t<-(yt$stop)^0.3*yt$x3
# yt$x3t<-1/(yt$stop)^1.5*yt$x3
# yt$x3t<-1/(yt$stop)^1.2*yt$x3
# yt$x3t<-log(yt$stop)*yt$x3
...
#   abline(h=0, col= "red")
# }
# #yt$x3t<-1/(yt$stop)^1.5*yt$x3 fonctionne!
# 
# transf <- "1/t^1.5"
...
#   HRIC <- round(c(exp(m), exp(m + qnorm(0.975)*sqrt(variance) * c(-1,1))),3)
#   HRIC <- paste0(HRIC[1], " [", HRIC[2], " - ", HRIC[3],"]")
#   dfHR <- rbind(dfHR,data.frame(transf = transf, param = paste0(i, "; ", i,"t"), beta = paste(round(b2,3), collapse = ";"), HRIC = HRIC))
# }
# 
# #HRIC pour x2 : RP ok => pas de modif
# test <- summary(coxt)
# coefbeta <- b[grep("x2", names(b))]
# stat <- round(test$robscore["test"],2)
# pval <- round(test$robscore["pvalue"],4)
# 
# transf <- NA
# tps_clinique <- NA
# HRIC <- round(data.frame(IC=test$conf.int["x2",1], l=test$conf.int["x2",3], u=test$conf.int["x2",4]),3)
# HR <- HRIC$IC
# HRIC <- paste0(HRIC[,1], "[", HRIC[,2], "-", HRIC[,3], "]")
# dfHR <- rbind(data.frame(transf = NA, param = "x2", beta = paste(round(coefbeta,3), collapse = ";"), HRIC = HRIC), dfHR)
# 
# df1 <- data.frame(variable = var, recode = FALSE, RP = FALSE, transf = dfHR$transf, tps_clinique = tps_clinique, HRIC = dfHR$HRIC, test = "robust",
#                   statistic = stat, pvalue = pval, param = dfHR$param, beta = dfHR$beta, stringsAsFactors = FALSE)
# 
...
HRIC <- lapply(c("x2", "x3"), function(lev){
  coefbeta2 <- coefbeta[grep(lev, names(coefbeta))]
  vcov2 <- diag(vcov(coxt))[grep(lev, names(coefbeta))]
  HRIC <- round(exp(cbind(coefbeta2[i], qnorm(0.025, coefbeta2[i],
                                              sqrt(vcov2[i])), qnorm(1-0.025, coefbeta2[i], sqrt(vcov2[i])))),3)
...
df1 <-data.frame(variable = var, recode = FALSE, RP = FALSE,
                            transf = transf, tps_clinique = .tps_clinique, HRIC = dfHR$HRIC, test = "robust",
                            statistic = stat, pvalue = pval, param = dfHR$name_param, beta = dfHR$coefbetacol)#HRIC : sans transformation uniquement pour l'instant
...
ztr
for (i in seq_along(bt)) {
  plot(zti[i], ylim = c(-5,5), main = var)
  title(names(bt)[i])
  abline(h=bt[i], col="blue")
  abline(h=0, col="red")
  abline(v=b)
...
df <-data.frame(variable = var, recode = FALSE, RP = FALSE,
                 transf = transf, tps_clinique = .tps_clinique, HRIC = dfHR$HRIC, test = "robust",
                 statistic = stat, pvalue = pval, param = dfHR$name_param, beta = dfHR$coefbetacol)#HRIC : sans transformation uniquement pour l'instant
...
# # yt$x2t<-1/(yt$stop^1.5)*yt$x2
# # yt$x2t<-1/(yt$stop^2)*yt$x2
# # yt$x2t<-(yt$stop)*yt$x2
# # yt$x2t<-(yt$stop)^0.3*yt$x2
# # yt$x2t<-log(yt$stop)*yt$x2
# # yt$x2t<- yt$stop^2*yt$x2
# # yt$x2t<- yt$stop^2.5*yt$x2
# yt$x2t<- yt$stop^4*yt$x2
# # yt$x2t<- sqrt(yt$stop)*yt$x2
# 
...
# #interpretation
# test <- summary(coxt)
# coefbeta <- round(test$coefficients[ ,"coef"], 3)
# name_param <- rownames(test$coefficients)
# stat <- round(test$robscore["test"],2)
# pval <- round(test$robscore["pvalue"],4)
# 
...
# HRIC <- round(exp(cbind(coef(coxt)[i], qnorm(0.025, coef(coxt)[i], sqrt(diag(vcov(coxt))[i])), qnorm(1-0.025, coef(coxt)[i], sqrt(diag(vcov(coxt))[i])))),3)
# HRIC <- paste0(HRIC[1], " [", HRIC[2], " - ", HRIC[3],"]")
# dfHR <- data.frame(tps_clinique = tps_clinique, transf = transf, param = paste(name_param[grep("x2", name_param)], collapse = ";"), beta = paste(coefbeta[grep("x2", names(coefbeta))], collapse = ";"), HRIC = HRIC, stringsAsFactors = FALSE)
# 
# 
# tps_clinique <- NA
# HRIC <- round(data.frame(IC=test$conf.int["x3",1], l=test$conf.int["x3",3], u=test$conf.int["x3",4]),3)
# HRIC <- paste0(HRIC[,1], "[", HRIC[,2], "-", HRIC[,3], "]")
# dfHR <- rbind(dfHR, data.frame(tps_clinique = NA, transf = NA, param = "x3", beta = coefbeta[grep("x3", names(coefbeta))], HRIC = HRIC, stringsAsFactors = FALSE))
# 
# df <- data.frame(variable = var, recode = FALSE, RP = FALSE, transf = dfHR$transf, tps_clinique = tps_clinique, HRIC = dfHR$HRIC, test = "robust",
#                  statistic = stat, pvalue = pval, param = dfHR$param, beta = dfHR$beta, stringsAsFactors = FALSE)
# 
...
ztr
for (i in seq_along(bt)) {
  plot(zti[i], ylim = c(-2,2), main = var)
  title(names(bt)[i])
  abline(h=bt[i], col="blue")
  abline(h=0, col="red")
  abline(v=b)
...
#                  statistic = stat, pvalue = pval, param = myParam, beta = mybeta, stringsAsFactors = FALSE)
# df1 <- rbind(df1,df)
...
# #par(mfrow=c(2,2))
# for (i in 1:4) {
#   plot(zti[i], ylim = c(-2,3), resid = FALSE)
#   title(names(b)[i])
#   abline(h=b[i])
#   abline(h=0, col = "red")
# }
...
#par(mfrow=c(2,2))
for (i in 1:4) {
  plot(zti[i], ylim = c(-1,1), resid = FALSE)
  title(names(b)[i])
  abline(h=b[i])
  abline(h=0, col = "red" )
}
...
for (i in "x2"){
  ...
  HRIC <- round(c(exp(m), exp(m + qnorm(0.975)*sqrt(variance) * c(-1,1))),3)
  HRIC <- paste0(HRIC[1], " [", HRIC[2], " - ", HRIC[3],"]")
  dfHR <- rbind(dfHR, data.frame(transf = transf, param = paste0(i, "; ", i,"t"), beta = paste(round(b2,3), collapse = ";"), HRIC = HRIC, stringsAsFactors = FALSE))
}
...
HR <- HRIC$IC
HRIC <- paste0(HRIC[,1], "[", HRIC[,2], "-", HRIC[,3], "]")
dfHR <- rbind(dfHR, data.frame(transf = NA, param = "x3", beta = paste(round(coefbeta,3), collapse = ";"), HRIC = HRIC, stringsAsFactors = FALSE))
...
# #transformation du temps pour x2
# yt$x1t<- (yt$stop)^1.7*yt$x1
# #yt$x3t<- (yt$stop)^2*yt$x3
# coxt<-coxph(Surv(start, stop, etat)~x1+x1t+x3+cluster(PATIENT), data=yt)
# #coxt<-coxph(Surv(start, stop, etat)~x2+x2t+x3+x3t+cluster(PATIENT), data=yt)
...
vat<-apply(expand.grid(list("x", c(1,3),"t",  1:(length(b)+1))),1, paste, collapse="")
vat <- vat [vat %in% names(yt)]
x<-yt[, vat]
sx<-colSums(x) #interval de temps sans evt
wat<-vat[sx>0] #on supprime interval de temps quand pas d'evenement
...
HRIC <- lapply(c("x1", "x3"), function(lev){
  coefbeta2 <- coefbeta[grep(lev, names(coefbeta))]
  vcov2 <- diag(vcov(coxt))[grep(lev, names(coefbeta))]
  HRIC <- round(exp(cbind(coefbeta2[i], qnorm(0.025, coefbeta2[i],
                                              sqrt(vcov2[i])), qnorm(1-0.025, coefbeta2[i], sqrt(vcov2[i])))),3)
...
# 
# df <- data.frame(variable = var, recode = FALSE, RP = TRUE, transf, tps_clinique = tps_clinique, HRIC, test = "robust",
#                  statistic = stat, pvalue = pval, param = names(coefbeta), beta = as.factor(coefbeta), stringsAsFactors = FALSE)
# 
# df1 <- rbind(df1,df)
...
data2 <- s1
data2$survIC <- paste0(data2$param, " : ", data2$survIC)
...
#------------
# comment when presenting all results
...
data$param <- gsub("^a", "", as.character(data$param))
# #merge qvec survie
# s5$param <- s5$group
...
#------------
#merge avec la survie et mise en ligne
data2 <- s5[s5$variable=="TOUX_EFFICACE" & s5$param != 2, ]
data2$survIC <- paste0(data2$param, " : ", data2$survIC)
data2$survIC <- paste(data2$survIC[data2$param==0], data2$survIC[data2$param==1], collapse = " ; ")
#data2 <- rbind(data2[data2$param == 1, ], s5[s5$variable=="LIEUDEB_recode", ])
data2 <- rbind(data2[data2$param == 1, ], s5[s5$variable=="LIEUDEB_rec2", ])
data2$variable <- as.character(data2$variable)
data2$survie <- data2$survIC
data$param_nu <- gsub("t1;", "", str_extract(data$param, "[[a-z]|_|1]*t1;"))
data <- merge(data, data2, by.x=c("variable", "param_nu"), by.y = c("variable", "param"), all = TRUE)
